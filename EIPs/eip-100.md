...
k개의 삼촌을 포함한 블록이 정확히 같은 타임스탬프를 가진 k+1개의 블록 집합과 동등하다고 가정하는 것과 오차가 크지 않다는 것을 확인하엿습니다. 그러나 정확한 엉클 블록의 갯수를 공식에 활용할 경우 블록 헤더만이 아닌 블록 전체에 의존해야 하는 문제가 발생합니다. 따라서 우리는 블록 헤더에만 의존하여 근사치를 추정하는 첫번째 공식을 사용합니다. 
